...
test.group('Catalog search performance', (group) => {
  ...
  test('keeps catalog server-response p95 within 500 ms at 10,000 records', async ({
    assert,
    client,
  }) => {
    ...
    for (let offset = 0; offset < ACTIVE_RECORD_COUNT; offset += 500) {
      ...
      await ProductVariant.createMany(
        Array.from({ length: count }, (_, localIndex) => {
          const index = offset + localIndex
          return {
            publicId: productVariantPublicId(index),
            productId: product.id,
            name: `Performance target Variant ${String(index).padStart(5, '0')}`,
            status: 'active' as const,
          }
        })
      )
    ...
    const productVariantP95 = await measureP95(async () => {
      const response = await client
        .get('/bills-of-materials/product-variant-candidates?search=performance%20target')
        .header('Authorization', `Bearer ${session.token}`)
      response.assertStatus(200)
      assert.lengthOf(response.body().items, 25)
      assert.isTrue(response.body().hasMore)
    ...
    assert.isAtMost(
      productVariantP95,
      TARGET_P95_MILLISECONDS,
      `Product Variant p95 was ${productVariantP95} ms`
    )
  }).timeout(120_000)
})
...
function patternSetPublicId(index: number) {
  ...
  for (let position = 0; position < 6; position += 1) {
    suffix = PUBLIC_ID_ALPHABET[remaining % PUBLIC_ID_ALPHABET.length] + suffix
    remaining = Math.floor(remaining / PUBLIC_ID_ALPHABET.length)
  }
  return `PS-${suffix}`
}

function productVariantPublicId(index: number) {
  let remaining = index
  let suffix = ''
  for (let position = 0; position < 6; position += 1) {
    suffix = PUBLIC_ID_ALPHABET[remaining % PUBLIC_ID_ALPHABET.length] + suffix
